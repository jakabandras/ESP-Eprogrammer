...
TFT_File::TFT_File(TFT_eSPI &tfts, Joystick &joystick, uint8_t textSize, String fileFilter, String getDir)
...
void TFT_File::setActDir(String actdir)
{
	actDir = actdir;
}

void TFT_File::resetDir()
{
	actDir = "/";
}

// Ezt kellfolytatni, egylőre csak másoltam
// be kell tudnia lépni az alkönyvtárakba és választás esetén
// a választott fájl teljes élérésé útját a névvel együtt adja vissza
String TFT_File::showFiles(MENU menu[], int8_t active)
{
	uint8_t menuCount = 0;
	uint8_t first = 1;
	uint8_t lfirst = 0;
	...
	return menu[current].option;
}
void TFT_File::readFiles()
{
	fs::File dir, file;
	dir = FFat.open(actDir);
	int cnt = 0;

	items.push_back((MENU){"Flash tartalom:", cnt++});
	if (actDir != "/")
		items.push_back((MENU){"..", cnt++});
	if (dir.isDirectory())
	{
...
void TFT_File::processDir(String getDir)
{
}
